...
<script>
  ...
  const getRfid4gczBasicInfo = function (params) {
  ...
  export default {
    ...
    data() {
      return {
        ...
        option: {
          supportRemind: true,
          gridManagerName: 'Rfid4gczBasicInfo',
          height: '572px',
          ...
          ajax_data: (settings, params) => {
            ...
            console.log(params)
            return getRfid4gczBasicInfo(params)
          },
          ajax_type: 'POST',
          supportMenu: true,
          query: {
    ...
    methods: {
      onSearch: function () {
        // const device_type = parseInt(this.$store.state.sblb)
        const device_type = parseInt(this.$store.getters.getSblb)
        if(this.$store.getters.getSblb===2){
